/**
 * Markdown implementation based on 'marked' from Jeffrey.
 *
...
function Lexer(options) {
  ...
  if (this.options.gfm) {
    var gfm = merge({},block.gfm);
    if (!this.options.todo) delete gfm.todo;
    if (!this.options.tables){ delete gfm.table; delete gfm.nptable};

    merge(this.rules, gfm);
  }
  
  if (this.options.marktex) {
    var marktex = merge({},block.marktex);
    if(!this.options.math) delete marktex.math_fences;
    if(!this.options.smartlist) delete marktex.list;
    if(!this.options.align) delete marktex.aligned_paragraph;

    merge(this.rules, marktex);
  }
}
...
function marktex(src, opt, callback) {
  ...
  try {
    ...
    e.message += '\nPlease report this to https://github.com/buddys/marktex.';
